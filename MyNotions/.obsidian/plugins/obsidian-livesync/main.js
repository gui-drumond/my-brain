...
var ObsidianLiveSyncSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    const b = s.createDiv(), E = b.createEl("h3", {
      ...
      text: ""
    }), S = "0.22.4", L = ~~(versionNumberString2Number(S) / 1e3), O = createSpan();
...
async function getDBEntryFromMeta(s, r, o, u = false, g = true, _ = false) {
  ...
  if ("newnote" == r.type || "plain" == r.type) try {
    ...
    let _ = [];
    const m = Math.min(10, Math.ceil(r.children.length / 10)) + 1, b = s.settings.doNotPaceReplication ? () => {} : await globalConcurrencyController.acquire(m);
    try {
      if (s.settings.readChunksOnline) {
        ...
        if (u) for (const g of u) if (g && "leaf" == g.type) _.push(g.data); else {
          ...
          if (o) Logger(`Could not retrieve chunks of ${S} (${r._id.substring(0, 8)}). we have to `, LOG_LEVEL_NOTICE);
          return false;
        }
      } else try {
        if (g) {
          ...
          if (o.rows.some((s => "error" in s))) {
            ...
            Logger(`Could not retrieve chunks of ${S}(${r._id.substring(0, 8)}). Chunks are missing:${s}`, LOG_LEVEL_NOTICE);
            return false;
          }
          if (o.rows.some((s => s.doc && "leaf" != s.doc.type))) {
            const s = o.rows.filter((s => s.doc && "leaf" != s.doc.type)).map((s => s.id)).join(", ");
            Logger(`Could not retrieve chunks of ${S}(${r._id.substring(0, 8)}). corrupted chunks::${s}`, LOG_LEVEL_NOTICE);
            return false;
          }
    ...
    if ("undefined" != typeof s.corruptedEntries[E._id]) delete s.corruptedEntries[E._id];
    return E;
  } catch (s) {
    if (isErrorOfMissingDoc(s)) {
...
var SerializedFileAccess = class {
  ...
  async vaultModify(s, r, o) {
    if ("string" == typeof r) return await processWriteFile(s, (async () => {
      ...
      if (r === u) {
        markChangesAreSame(s, s.stat.mtime, o.mtime);
        return false;
      }
...
var SETTING_HEADER = "````yaml:livesync-setting\n", SETTING_FOOTER = "\n````", ObsidianLiveSyncPlugin = class extends import_obsidian.Plugin {
  ...
  watchWorkspaceOpen(s) {
    if (!this.settings.suspendFileWatching) if (this.isReady) if (s) scheduleTask("watch-workspace-open", 500, (() => fireAndForget((() => this.watchWorkspaceOpenAsync(s)))));
  }
  ...
  async processEntryDoc(s, r, o) {
    ...
    const w = "newnote" == E.datatype ? decodeBinary(E.data) : getDocData(E.data);
    await this.ensureDirectoryEx(g);
    try {
      let s, o = true;
      if (!r) {
  ...
  applyStatusBarText(s, r) {
    ...
    const _ = ".CodeMirror-wrap,.markdown-preview-view.cm-s-obsidian,.markdown-source-view.cm-s-obsidian,.canvas-wrapper,.empty-state";
    if (this.settings.showStatusOnEditor) activeDocument.documentElement.querySelectorAll(_).forEach((s => s.setAttr("data-log", u + "\n" + g))); else activeDocument.documentElement.querySelectorAll(_).forEach((s => s.setAttr("data-log", "")));
  ...
  async syncAllFiles(s) {
    ...
    if (!r) {
      const s = new QueueProcessor((async s => {
        const r = s[0];
        return [ {
          file: r,
      ...
      s.pipeTo(new QueueProcessor((async s => {
        const r = (await this.localDatabase.allDocsRaw({
          keys: s.map((s => s.id)),
          include_docs: true
        })).rows.reduce(((s, r) => ({
        ...
        const o = s[0];
        await this.syncFileBetweenDBandStorage(o.file, o.doc, r);
      }), {
        batchSize: 1,
        concurrentLimit: 5,
        delay: 10,
  ...
  async syncFileBetweenDBandStorage(s, r, o) {
